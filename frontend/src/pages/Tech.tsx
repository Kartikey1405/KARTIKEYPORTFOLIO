...
import { 
  ...
  SiDocker, SiPlotly // Using Plotly as a standard fallback for Matplotlib if needed
} from 'react-icons/si';
...
const Tech = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-6 py-24 relative overflow-hidden">
      ...
      {/* 2. MAIN CONSTELLATION CONTAINER */}
      <div
        ref={containerRef}
        className="relative w-full max-w-6xl aspect-[16/10] overflow-hidden rounded-2xl z-20"
        style={{
...
// import { useState, useEffect, useRef } from 'react';
// import { motion } from 'framer-motion';
// // Corrected Imports - Using both Si and Fa for maximum compatibility
// import { 
//   SiReact, SiSpring, SiSpringboot, SiPython, SiTailwindcss, SiHtml5, SiCss3, SiJavascript, 
//   SiThymeleaf, SiMysql, SiMongodb, SiPostgresql, SiFastapi, SiNumpy, SiPandas, 
//   SiScikitlearn, SiVercel, SiRender, SiIntellijidea, SiEclipseide, SiGit, SiGithub,
//   SiDocker
// } from 'react-icons/si';
// import { FaJava, FaGithub } from 'react-icons/fa'; // Fallback for GitHub
// import { VscVscode } from "react-icons/vsc"; 
...
//   { id: 'js', name: 'JavaScript', icon: SiJavascript, x: 58, y: 62, size: 'core', color: '#F7DF1E' },
//   { id: 'springboot', name: 'Spring Boot', icon: SiSpringboot, x: 72, y: 42, size: 'core', color: '#6DB33F' },
//   { id: 'github', name: 'GitHub', icon: FaGithub, x: 50, y: 22, size: 'core', color: '#ffffff' },
//   { id: 'react', name: 'React', icon: SiReact, x: 32, y: 52, size: 'core', color: '#61DAFB' },
...
//   { id: 'mysql', name: 'MySQL', icon: SiMysql, x: 60, y: 78, size: 'satellite', color: '#4479A1' },
//   { id: 'postgres', name: 'PostgreSQL', icon: SiPostgresql, x: 68, y: 72, size: 'satellite', color: '#4169E1' },
//   { id: 'mongo', name: 'MongoDB', icon: SiMongodb, x: 75, y: 82, size: 'satellite', color: '#47A248' },
...
//   { from: 'thymeleaf', to: 'springboot' }, { from: 'python', to: 'fastapi' },
//   { from: 'python', to: 'numpy' }, { from: 'python', to: 'pandas' },
//   { from: 'numpy', to: 'scikit' }, { from: 'springboot', to: 'postgres' }, 
//   { from: 'springboot', to: 'mysql' }, { from: 'springboot', to: 'mongo' }, 
//   { from: 'git', to: 'github' }, { from: 'react', to: 'vercel' }, 
//   { from: 'springboot', to: 'render' },
// ];
...
//   return (
//     <div className="min-h-screen flex flex-col items-center justify-center px-6 py-24 relative overflow-hidden">
//       <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8 z-30">
//         <h1 className="text-4xl font-display font-bold text-glow-cyan mb-2">TECH CONSTELLATION</h1>
//         <p className="text-sm text-muted-foreground font-mono">Interactive star map of technologies</p>
//       </motion.div>

//       <div ref={containerRef} className="relative w-full max-w-6xl aspect-[16/10] overflow-hidden rounded-2xl z-20"
//         style={{ background: 'radial-gradient(ellipse at center, #0a0e1a 0%, #020408 100%)' }}>
        
//         {/* Background Effects */}
//         <div className="absolute inset-0 overflow-hidden pointer-events-none">
//           <motion.div className="absolute w-[500px] h-[500px] rounded-full blur-[120px] opacity-20"
//             style={{ background: 'radial-gradient(circle, #8B5CF6 0%, transparent 70%)', left: '5%', top: '10%' }}
//             animate={getParallaxOffset(0.3)} />
//           <motion.div className="absolute w-[450px] h-[450px] rounded-full blur-[100px] opacity-15"
//             style={{ background: 'radial-gradient(circle, #00F0FF 0%, transparent 70%)', right: '10%', bottom: '15%' }}
//             animate={getParallaxOffset(0.4)} />
//         </div>
...
//                 stroke={isHighlighted ? '#00F0FF' : '#ffffff20'} strokeWidth={isHighlighted ? 2 : 1} />
//             );
//           })}
//         </svg>
...
//             return (
//               <motion.div key={node.id} className="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer z-30"
//                 style={{ left: `${pos.x}%`, top: `${pos.y}%` }}
//                 onMouseEnter={() => setHoveredNode(node.id)} onMouseLeave={() => setHoveredNode(null)}>
//                 <motion.div className="flex flex-col items-center gap-2" animate={{ scale: isHovered ? 1.4 : isConnected ? 1.2 : 1 }}>
//                   <div className="rounded-full flex items-center justify-center border border-white/20 shadow-lg"
//                     style={{ width: size, height: size, background: `radial-gradient(circle at 30% 30%, ${node.color}, ${node.color}60)`, boxShadow: `0 0 20px ${node.color}40` }}>
//                     <node.icon size={size * 0.55} color="#ffffff" />
//                   </div>
//                   <span className={`text-[11px] font-mono font-bold whitespace-nowrap drop-shadow-lg transition-opacity ${isHovered || isConnected ? 'opacity-100' : 'opacity-60'}`}
//                     style={{ color: isHovered ? node.color : '#e2e8f0' }}>{node.name}</span>
//                 </motion.div>
//               </motion.div>
//             );
//           })}
